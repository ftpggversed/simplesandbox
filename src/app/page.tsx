        ...
        <header className="flex items-center justify-between p-4 bg-transparent backdrop-blur-sm">
          ...
          <div className="flex items-center space-x-2">
            ...
            <div className="relative">
              ...
              {showDownloadMenu && (<div className="absolute right-0 mt-2 w-40 bg-gray-800 rounded-md shadow-lg ring-1 ring-black ring-opacity-20 z-10">
            ...
            {[{label:'HTML',value:html,onChange:setHtml,mode:'html'},{label:'CSS',value:css,onChange:setCss,mode:'css'},{label:'JavaScript',value:js,onChange:setJs,mode:'javascript'}].map((e,i)=>(
              <div key={i} className="flex flex-col bg-gray-800 rounded-xl shadow-md border border-gray-700">
                ...
                <div className="flex-1">
                  <AceEditor mode={e.mode} theme="twilight" value={e.value} onChange={e.onChange}
                    name={`${e.mode}Editor`} editorProps={{ $blockScrolling: true }}
                    setOptions={{ showLineNumbers:true, tabSize:2, useWorker:false }} width="100%" height="100%"
                    fontSize={aceFontSize} style={{ background:'transparent' }} />
                </div>
              </div>
            ))}
          </div>
